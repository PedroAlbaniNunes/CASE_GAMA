...
/**
 * REST controller for managing {@link com.mycompany.mycase.domain.Meta}.
 */
@RestController
...
public class MetaResource {
     ...
     */
    @PutMapping("/{id}")
    public ResponseEntity<Meta> updateMeta(@PathVariable(value = "id", required = false) final Long id, @RequestBody Meta meta)
    ...
    public ResponseEntity<Meta> partialUpdateMeta(@PathVariable(value = "id", required = false) final Long id, @RequestBody Meta meta)
        throws URISyntaxException {
        ...
        Optional<Meta> result = metaRepository
            .findById(meta.getId())
            .map(existingMeta -> {
                if (meta.getNotaAnterior() != null) {
                    existingMeta.setNotaAnterior(meta.getNotaAnterior());
                }
